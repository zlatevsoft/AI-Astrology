...
function isPlausibleName(s: string) {
  const t = s.trim()
  if (t.length < 1 || t.length > 100) return false
...
export const birthDataSchema = z.object({
  name: z
    ...
    .refine((s) => isPlausibleName(s), {
      message: 'Name can only include letters (any language), numbers, and basic punctuation',
    }),
  ...
  time: z
    ...
    .regex(
      ...
      'Time must be in HH:MM (optional :ss), 24-hour'
    ),
  ...
  location: z
    ...
    .refine((s) => {
      ...
      if (/[<>]/.test(s)) return false
      // Any script (Cyrillic, Latin…), numbers, address punctuation — no control chars
      if (/[\u0000-\u0008\u000B\u000C\u000E-\u001F]/.test(s)) return false
      return true
...
export const partnerDataSchema = z.object({
  partnerName: z
    ...
    .refine((s) => isPlausibleName(s), {
      message: 'Partner name can only include letters (any language), numbers, and basic punctuation',
    }),
  ...
  partnerDate: z
    ...
    .refine(
      ...
      { message: 'Partner birth date must be between 1900 and today' }
    ),
  ...
  partnerTime: z
    ...
    .regex(
      /^([01]?[0-9]|2[0-3]):[0-5][0-9](?::[0-5][0-9])?$/,
      'Partner time must be in HH:MM (optional :ss), 24-hour'
    ),
  ...
  partnerLocation: z
    ...
    .refine(
      ...
      { message: 'Partner location contains invalid characters' }
    ),
})
...
// Validate and sanitize partner data (accepts chart `birthData` shape: name, date, time, location)
export function validateAndSanitizePartnerData(data: Record<string, unknown>) {
  const partnerName = (data.partnerName ?? data.name ?? '') as string
  const partnerDate = (data.partnerDate ?? data.date ?? '') as string
  const partnerTime = (data.partnerTime ?? data.time ?? '') as string
  const partnerLocation = (data.partnerLocation ?? data.location ?? '') as string

  const sanitizedData = {
    partnerName: sanitizeInput(partnerName),
    partnerDate,
    partnerTime,
    partnerLocation: sanitizeInput(partnerLocation),
  }
