...
import { featuresSection, featureCards } from '@/lib/home-sections-locale'
...
const FEATURE_META: {
  ...
  { icon: ChartBarIcon, color: 'from-blue-500 to-purple-600' },
  { icon: SparklesIcon, color: 'from-purple-500 to-pink-600' },
  { icon: ShieldCheckIcon, color: 'from-green-500 to-teal-600' },
  { icon: BoltIcon, color: 'from-yellow-500 to-orange-600' },
  { icon: HeartIcon, color: 'from-pink-500 to-rose-600' },
  { icon: GlobeAltIcon, color: 'from-indigo-500 to-blue-600' },
  { icon: DevicePhoneMobileIcon, color: 'from-cyan-500 to-blue-600' },
  { icon: LockClosedIcon, color: 'from-emerald-500 to-green-600' },
]

export function FeaturesSection() {
  const locale = useSiteLocale()
  const h = featuresSection[locale]
  ...
  return (
    <section id="features" className="py-20 bg-gradient-to-b from-white to-cosmic-50 dark:from-cosmic-900 dark:to-cosmic-800 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
          ...
          {features.map((feature, index) => (
            ...
            >
              <div className="bg-white dark:bg-cosmic-800 rounded-2xl p-6 h-full shadow-lg hover:shadow-xl transition-all duration-300 border border-cosmic-100 dark:border-cosmic-700 group-hover:border-cosmic-300 dark:group-hover:border-cosmic-600 overflow-hidden">
                ...
                <h3 className="text-lg font-semibold text-cosmic-900 dark:text-white mb-2">{feature.title}</h3>
                <p className="text-cosmic-600 dark:text-cosmic-300 text-sm leading-relaxed">{feature.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        >
          <p className="text-lg text-cosmic-600 dark:text-cosmic-300 mb-6">{h.bottomLine}</p>
          <a href="/pricing">
